...
%As an entry point to the PG, we define serialization based on the PG model.
%G2GML implementation converts the RDF dataset into a property graph dataset. The new dataset can also be saved in specific formats for loading into major graph database implementations.
...
%A G2GML description is a function that maps resources from an RDF graph to a set of PG nodes and edges that are mapped by all node and edge maps.
